...
class SyncManager(private val context: Context) {
            ...
            try {
                ...
                if (response.isSuccessful && response.body()?.code == 200) {
                    ...
                    if (syncResponse != null) {
                        val uploadErrors = syncResponse.uploadStatus?.errors.orEmpty()
                        val downloadErrors = syncResponse.downloadStatus?.errors.orEmpty()
                        val errors = uploadErrors + downloadErrors
                        if (errors.isNotEmpty()) {
                            val retryableCount = errors.count { it.retryable }
                            val summary = errors.joinToString("；") { "${it.errorMessage}（${it.entityType}:${it.localId}）" }
                            Log.w(TAG, "同步存在结构化错误: upload=${uploadErrors.size}, download=${downloadErrors.size}")
                            return@withContext Result.failure(
                                Exception("同步部分失败：$summary；可重试项 ${retryableCount} 条")
                            )
                        }
    ...
    private suspend fun saveDownloadedDietRecords(localUserId: Long, records: List<DietRecordData>?) {
        ...
        for (data in records) {
            ...
            val localRef = if (data.agentActionId != null && data.agentActionId > 0) {
                dietRecordDao.insertBatchIdempotent(
                    userId = localUserId,
                    actionId = data.agentActionId,
                    records = listOf(record)
                )
            } else {
                dietRecordDao.insert(record).toString()
            }
            Log.d(TAG, "合并饮食记录: food=${data.foodName}, actionId=${data.agentActionId}, recordKey=${data.recordKey}, localRef=$localRef")
        }
    }
    ...
    private suspend fun saveDownloadedTrainingPlans(localUserId: Long, plans: List<TrainingPlanData>?) {
        ...
        for (data in plans) {
            val plan = TrainingPlan(
                id = 0,
                userId = localUserId,
                // 普通计划必须保持 null，不能伪造一个时间戳作为 Agent ID
                agentActionId = data.agentActionId,
                title = data.title,
            ...
            val localId = trainingPlanDao.insertPlanIdempotent(plan)
            Log.d(TAG, "合并训练计划: title=${data.title}, actionId=${data.agentActionId}, localId=$localId")
        }
    }
