...
class ChatAdapter : RecyclerView.Adapter<ChatAdapter.ViewHolder>() {
    ...
    fun appendToLastMessageDirect(chunk: String) {
        if (messages.isNotEmpty()) {
            ...
            val newContent = currentMessage.content + chunk
            messages[lastIndex] = currentMessage.copy(
                content = newContent,
                isLoading = false
            )
            // 直接更新TextView（如果ViewHolder已绑定）
            // 如果ViewHolder未绑定，notifyItemChanged会在下次滚动时更新
            lastMessageViewHolder?.updateText(newContent)
        }
    }
